...
public struct MainView: View {
  ...
  @ObservedObject var viewModel: ChatViewModel
  @State private var newSessionId: String? = nil
  @State private var path: [String] = [] // Use a navigation path
  @Environment(\.modelContext) var modelContext
  @Environment(\.dismiss) var dismiss
  private var bgcolors: Color
  var backgroundImage: UIImage?
  ...
  public var body: some View {
    NavigationStack(path: $path) {
      ...
      .navigationDestination(for: String.self) { sessionId in
        NewSessionView(session: sessionId, viewModel: viewModel, backgroundImage: backgroundImage)
          .modelContext(modelContext)
      }
    }
        ...
        List {
          ForEach(thread) { thread in
            ...
            MessageSubView(thread.title, dateString)
              .onTapGesture {
                path.append(thread.sessionId) // Navigate to the session
              }
              .listRowSeparator(.hidden)
              .listRowBackground(Color.clear)
              .listRowInsets(EdgeInsets(top: 4, leading: 0, bottom: 4, trailing: 0))
      ...
      HStack(spacing: 6) {
        ...
        Button(action: {
          ...
          if let newSessionId = newSessionId {
            path.append(newSessionId) // Navigate to the new session
          }
        }) {
          if let newChatButtonImage = SetUIComponents.shared.newChatButtonImage {
            Image(uiImage: newChatButtonImage)
